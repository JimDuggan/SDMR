...
o<-as_data_frame(data.frame(ode(y=stocks,times=simtime,func=model,parms=auxs,method='euler')))

s1 <- select(o,time,c(FStock,DStock,WStock,RStock))
ts1 <- gather(s1,key=Stock,value=Amount,FStock:RStock)
ts1 <- mutate(ts1,Entity=Stock)
ts1$Entity<- factor(ts1$Entity,
                      levels=c("FStock","DStock","WStock","RStock"))


ggplot(ts1)+geom_area(alpha=1,aes(x=time,y=Amount,fill=Entity))+
  ggtitle("Supply Chain Dynamics")+
  xlab("Week Number")+ylab("Inventory")

ggplot(ts1)+geom_path(alpha=1,aes(x=time,y=Amount,colour=Entity))+
  geom_point(aes(x=time,y=Amount))+
  ggtitle("Supply Chain Dynamics")+
  xlab("Week Number")+ylab("Inventory")
